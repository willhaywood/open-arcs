...
import type { FactionId, Weights } from '@arcs/engine'

interface CollectJob {
  /** Evaluator weights the *collecting* bot plays with; omitted means the hand-set ones. */
...
/*
 * Data is generated by the policy being fitted, which is what makes this policy *iteration* rather
 * than a single regression: round two sees positions round one's weights actually reach.
 */
const bot = job.weights === undefined ? heuristicBot : heuristicBotWith(job.weights as Weights)
...
for (let g = job.shard; g < job.games; g += job.jobs) {
  ...
  for (; steps < MAX_STEPS; steps++) {
    ...
    const step = stepBot(r, bot, who, reg, asked)
    r = step.result
    asked = step.asked
  }
